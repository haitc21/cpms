...
- CPMS is the provider-neutral control plane and PostgreSQL source of truth.
- CPMS must not import OpenStackSDK or encode Nova/Neutron/Cinder/Glance behavior.
- OSPS is accessed through versioned contracts and RabbitMQ, except the internal credential-resolution boundary.
...
- Valkey and MongoDB are not CPMS runtime dependencies in the current scope.
...
2. Change canonical Pydantic/OpenAPI/JSON Schema in CPMS.
3. Preserve compatibility or explicitly increment `schema_version`.
4. Update golden fixtures and checksum manifest.
5. Coordinate the pinned OSPS copy and its tests in the same sprint.
...
- CPMS uses FastAPI, Pydantic v2, SQLAlchemy 2, Alembic, Psycopg 3, RabbitMQ/aio-pika; it does not depend on OpenStackSDK.
