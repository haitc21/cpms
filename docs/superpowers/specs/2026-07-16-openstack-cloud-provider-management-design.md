...
**Date:** 2026-07-16  
**Services:** Cloud Provider Management Service (CPMS), OpenStack Provider Service (OSPS)

## 1. Purpose and first delivery scope

CPMS is the provider-neutral control plane for cloud providers. OSPS is a stateless OpenStack adapter. The first delivery supports OpenStack without coupling the common contracts to OpenStackSDK or prematurely implementing VMware behavior.
...
- REST APIs in CPMS and limited internal APIs in CPMS/OSPS.
...
CPMS owns the public API, common contracts, provider configuration, credentials, normalized inventory, operations, and PostgreSQL persistence. OSPS owns OpenStack connectivity, discovery, collection, mapping, VM execution, waiters, and OpenStack error translation. Long-running work uses RabbitMQ. PostgreSQL is CPMS's source of truth; OSPS has no business database.

```text
Client --REST--> CPMS --commands/inventory/results--> RabbitMQ <--consume/publish-- OSPS
                   |                                                     |
...
- An OSPS inventory database creates two sources of truth and violates the stateless-service requirement.
...
- Store encrypted credentials and resolve credential references for authorized OSPS calls.
...
CPMS never imports OpenStackSDK types and does not contain Nova, Neutron, Cinder, Glance, or Keystone business logic.

### 3.2 OSPS

- Consume OpenStack commands and resolve credentials from CPMS.
...
OSPS stores no credential, inventory, tenant, product, pricing, billing, or audit state.

### 3.3 Deployment topology

- CPMS and OSPS deploy and scale independently.
- They use the shared CMP RabbitMQ and PostgreSQL clusters. CPMS owns a separate database or schema and DB user.
- Exchanges, queues, dead-letter queues, routing keys, and permissions are isolated by domain/service.
- OSPS does not require PostgreSQL, MongoDB, or Valkey.
- CPMS readiness checks PostgreSQL and RabbitMQ. OSPS readiness checks RabbitMQ. A failed customer OpenStack connection affects only that connection's status.
...
Credentials are stored in CPMS PostgreSQL for the first delivery. Sensitive values are encrypted at the application layer using a deployment key held outside the database. Public APIs never return passwords. Logs, tracing attributes, RabbitMQ messages, operation result payloads, and inventory must redact password, token, authorization header, CA private material, and VM `user_data`.

Commands contain only `credential_reference`. OSPS resolves it through CPMS internal REST immediately before use and retains the clear value only in process memory. This boundary permits a later secret-store implementation without changing command contracts.
...
OSPS must not lock to an OpenStack release or a fixed maximum microversion. It performs version and service discovery for each connection and reports a capability document to CPMS.
...
OSPS uses the minimum microversion required by a selected feature, bounded by discovery. Unsupported functionality fails before provider mutation with `CAPABILITY_NOT_SUPPORTED`. Discovery failures do not make the OSPS process unready.

## 6. Common resource model

Each inventory table has a CPMS UUID and provider identity. Names are never identities.
...
Relationships use CPMS UUIDs when available and retain provider IDs during ingestion so out-of-order batches can be resolved at finalization. Join tables represent instance-port and instance-volume relationships, including device, boot index, and delete-on-termination where known.
...
CPMS creates an operation and `inventory_sync`, publishes `inventory.collect`, and enforces at most one active full sync per connection. OSPS collects each supported resource with OpenStackSDK pagination, maps it, and publishes batches. Logical collection order is region/project, flavor/image, network/subnet, port/volume, then instance, but CPMS remains safe under out-of-order delivery.

Each batch contains `sync_id`, `resource_type`, per-type `sequence`, `is_last`, `item_count`, `checksum`, and items. CPMS upserts by provider identity and sets `last_sync_id`. It finalizes relationships only after required collections close.

Deletion reconciliation occurs only when:

- OSPS publishes successful completion;
...
Full sync can be started manually or on a CPMS-owned schedule with jitter. Schedule frequency is configuration, not a hard-coded product rule.

### 7.2 Refresh after operations and manual targeted refresh

After create/start/stop/reboot, OSPS waits for the desired provider state and returns a normalized instance snapshot plus affected ports/volumes. CPMS updates operation and inventory atomically. After delete, OSPS waits for server absence and emits a tombstone, then refreshes affected ports/volumes.

A manual targeted-refresh endpoint accepts a supported resource type and CPMS/provider resource ID. Provider `NotFound` produces a tombstone. Timeout, authorization failure, or service unavailability never imply deletion.

### 7.3 Deleted and reappearing resources

Deleted resources remain for audit and reconciliation, are hidden from lists by default, are available by ID or `include_deleted=true`, and are not physically purged by default. If the same provider ID reappears, CPMS reactivates the same row and UUID while preserving operation/event history.
...
CPMS validates common shape and inventory ownership. OSPS revalidates provider existence/scope immediately before mutation. OSPS base64-encodes `user_data` as required by Nova, never logs or inventories it, and enforces the safe/provider-discovered size limit.
...
Delete delegates root-volume behavior to Nova block-device `delete_on_termination`, matching ManageIQ's lifecycle pattern; OSPS does not blindly issue a second Cinder delete.
...
CPMS records every transition in `operation_events` with sequence, old/new state, safe details, message ID, and timestamp. This log supplies future LMS audit events. Optional `actor_context` reserves subject, organization, workspace, source service, and request context without applying authorization now.
...
- OSPS queue: `osps.command.v1`
- CPMS event queue: `cpms.cloud.event.v1`
- OSPS DLQ: `osps.command.dlq.v1`
- CPMS DLQ: `cpms.cloud.event.dlq.v1`
...
Messages and queues are durable; messages are persistent. Publishers use confirms. Consumers use manual acknowledgement and bounded prefetch. CPMS uses a transactional outbox in the same DB transaction as operation creation/state changes. CPMS event consumers use a transactional inbox. OSPS uses message identity plus operation identity and provider-side precondition checks to make processing safe under at-least-once delivery.
...
CPMS enforces unique `(provider_connection_id, operation_type, idempotency_key)`. Repeating the same key and semantically equal request returns the existing operation. Reusing the key with different input returns `409 IDEMPOTENCY_KEY_REUSED`.

CPMS inbox deduplicates `(consumer_name, message_id)`. Inventory batches also enforce `(sync_id, resource_type, sequence)`. A duplicate with a different checksum is a contract/integrity failure.

OSPS keeps no database. For retries it uses operation ID, provider resource ID where known, deterministic metadata tags where supported, and provider-state preconditions. Create places the CPMS operation ID/idempotency marker in permitted server metadata and searches for an existing matching server before retrying creation. A result publish failure causes command redelivery; OSPS observes the existing provider state and republishes the same terminal result instead of repeating the mutation.
...
OSPS maps the OpenStackSDK exception hierarchy and safe response data. Raw response bodies are not returned indiscriminately. HTTP/provider request IDs are preserved for support.
...
Timeouts are layered and configurable: connect/read request timeout, per-service operation timeout, waiter interval, total operation deadline, and message processing deadline. OSPS never waits indefinitely. CPMS reconciliation marks an operation timed out when no terminal event arrives by `timeout_at`; a later provider result is retained as a late event and requires deterministic reconciliation rather than silently rewriting a terminal state.
...
- `GET /internal/v1/credentials/{reference}` for OSPS credential resolution.
...
OSPS internal API is deliberately small:
...
Long-running provider work is not exposed as synchronous OSPS REST. RabbitMQ is the authoritative command path.

## 14. PostgreSQL model

CPMS uses SQLAlchemy and Alembic with typed tables rather than a single EAV inventory table.
...
CPMS unit tests cover validation, state transitions, idempotency-key reuse, lifecycle reconciliation, relation resolution, error serialization, crypto boundary, filtering/pagination, and outbox/inbox logic.

OSPS unit tests use SDK fakes/mocks for mapping every resource, pagination, capability discovery, exception normalization, retry classification, waiter terminal/timeout states, create payloads, volume lifecycle, redaction, and replay-safe handlers.
...
- Verify OSPS output is accepted by CPMS models.
...
CPMS integration tests use PostgreSQL and RabbitMQ containers for migrations, constraints, transactions, concurrent idempotency, outbox confirms, inbox duplicates, out-of-order batches, incomplete sync safety, and redelivery.

OSPS integration tests use RabbitMQ plus mocked HTTP/OpenStack endpoints to verify credential resolution, OpenStackSDK requests, retry/redelivery, provider state preconditions, and result publication.
...
8. Restart CPMS/OSPS during work and verify no lost operation, safe redelivery, and eventual terminal state.
...
- MS/TMS: bind organization/domain and workspace/project at CPMS, leaving OSPS provider-scoped.
- LMS: publish audit events derived from immutable operation events through an outbox.
- Secret store: replace CPMS credential repository implementation without changing command references.
