"""CPMS-001: application factory acceptance tests."""
...
def test_create_app_returns_fastapi_application() -> None:
    from cpms.main import create_app

    app = create_app()

    assert isinstance(app, FastAPI)
    assert app.title == "CPMS"
...
def test_cli_main_exposes_help(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "cpms" in captured.out.lower() or "usage" in captured.out.lower()
